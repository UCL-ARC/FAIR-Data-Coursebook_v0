...
Setting up the Workbench on your local system is a useful way to develop lessons without the need of an internet connection.

We recommend using `conda` to install the Workbench packages and dependencies as this will be the same process for all operating systems.

However, if you do not want to use `conda`, then we provide instructions to work through the install of all the required dependencies manually.

## conda-forge Install {#101-conda-forge}
...
### With Pixi {#101-conda-pixi}

[Pixi](https://pixi.prefix.dev/) environments (recommended) are fully reproducible by default.
...
## Separate Dependency Install

Depending on your operating system, there are different instructions to install the Workbench and all the required software manually. However, all installation methods will need [required](#101-required) software, and some [recommended](#101-recommend) packages.

Please follow the specific instructions for [Windows](#windows), [MacOS](#mac), or [UNIX/Linux/WSL](#linux) below.
